...
client = (settings['client-ip'], settings['client-port'])

s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
s.bind((host, port))


def parse_coords(sign, value):
    value = str(value)
    coords = str(sign + (value.zfill(3)))
    return coords
        ...
        if x != 0:
            ...
            if abs_x != 0:
                if abs_x > 999:
                    abs_x = 999
                if x > 0:
                    cords += parse_cords("X", abs_x)
                else:
                    cords += parse_cords("C", abs_x)
        ...
        if y != 0:
            ...
            if abs_y != 0:
                if abs_y > 999:
                    abs_y = 999
                if y > 0:
                    cords += parse_cords("Y", abs_y)
                else:
                    cords += parse_cords("U", abs_y)
